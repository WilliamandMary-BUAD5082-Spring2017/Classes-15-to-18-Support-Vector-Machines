...
rm(list=ls())
set.seed(1)
# The e1071 package contains the svm() function, which can be used to fit a 
# support vector classifier when the argument kernel="linear" is used. 
#
# This function uses a slightly different formulation from (9.14) and (9.25) for the
# support vector classifier - instead of a budget, a cost is used. In the text, a high
# budget meant a wider margin, whereas here, a high cost means a smaller margin (just
# the oppostite of the text's "C".
...
#
# Note that in order for the svm() function to perform classification (as 
# opposed to SVM-based # regression), we must encode the response as a factor. 
# 
...
# somewhat jagged in the plot. 
#
# We see that in this case only one observation  is misclassified. 
#    Note that here the second feature is plotted on the x-axis and the first feature is plotted 
#    on the y-axis, in contrast to the behavior of the usual plot() function in R.) 
# The support vectors are plotted as crosses and the remaining observations are 
#     plotted as circles; 
...
# We first further separate the two classes in our simulated data so that they are
# linearly separable:
x[y==1,]=x[y==1,]+0.5
